...
class SoftActorCritic(nn.Module):
    def __init__(
        ...
        if self.auto_tune_temperature:
            ...
            self.log_alpha = nn.Parameter(
                torch.tensor(
                    np.log(temperature), dtype=torch.float32, device=ptu.device
                )
            )
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            target_values = reward + self.discount * not_done * next_qs
            # ENDTODO
    ...
    def entropy(self, action_distribution: torch.distributions.Distribution):
        ...
        # rsample() is required so entropy gradients flow through the action into
        # μ, σ (detached .sample() has E[∇ log π] = 0 and cannot maximize H).
